...
- SHA-256：`2e5bb08f73d5c4964a41fbf52814a92d170b41b5d6e13483588466c0d270839f`
...
| 1920×1080 基准每行最大字符宽度 | `80` | `20` 到 `120` | 设置 1920×1080 横屏的基准值。其他横屏按“基准值 × 实际宽度 ÷ 1920”计算；竖屏直接使用基准值。修改基准值后，所有横屏分辨率同步重新计算。 |
...
| 双字幕上下间隔 | `0` | `0` 到 `80` | 普通双字幕使用相同的水平锚点。此值用于增加额外间隔，以 1080p 为基准；其他分辨率按视频高度同比例换算。 |
| 字幕位置 | 不修改原字幕位置 | 不修改原字幕位置、最底部居中 | 默认保留原 Style 对齐、边距和内联定位。「最底部居中」只调整普通对白，特效字幕始终保留原位置。 |
| 距最底部距离 | `20` | `0` 到 `540` | 仅在「最底部居中」模式下生效。以 1920×1080 的画面高度为基准，所有分辨率均按“设置值 × 实际高度 ÷ 1080”换算。 |
| 修复网页端全屏字幕位置 | 开启 | 开启、关闭 | 修复 Emby Web 进入全屏后 ASS 字幕画布仍使用窗口模式尺寸，导致字幕整体向上、向左移动的问题。插件只管理带自身标记的 CSS，不覆盖其他自定义 CSS。 |
...
| 字体边框（含单字幕） | 开启 | 开启、关闭 | 控制单字幕和双字幕第一行是否使用外边框。边框类型固定为外边框。 |
| 边框宽度（含单字幕） | `0.1` | `0` 到 `20` | 单字幕和双字幕第一行的外边框宽度。 |
| 边框颜色（含单字幕） | `#000000` | `#RRGGBB`、`#AARRGGBB` | 单字幕和双字幕第一行的外边框颜色，默认值为黑色。 |
...
| 向上偏移比例 | `60` | `0` 到 `200` | 按副字幕 Fontsize 的百分比向上移动副字幕。`0` 表示不偏移。 |
| 字体颜色 | `#FFD966` | `#RRGGBB`、`#AARRGGBB` | 用于双字幕第二行。八位格式中的前两位表示 Alpha。 |
| 字体风格 | 常规 | 常规、粗体、斜体、粗斜体 | 用于双字幕第二行。 |
| 字体边框 | 开启 | 开启、关闭 | 控制双字幕第二行是否使用外边框。边框类型固定为外边框。 |
| 边框宽度 | `0.1` | `0` 到 `20` | 双字幕第二行的外边框宽度。 |
| 边框颜色 | `#000000` | `#RRGGBB`、`#AARRGGBB` | 双字幕第二行的外边框颜色，默认值为黑色。 |
